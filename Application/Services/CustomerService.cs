...
namespace Application.Services.Implementation
{
    public class CustomerService : ICustomerService
    {
        ...
        public async Task<BaseResponse<RegisterCustomerResponse>> RegisterAsync(RegisterCustomerRequest request)
        {
            ...
            if (exist)
            {
                return BaseResponse<RegisterCustomerResponse>.Failure("Email already exists.");
            }
            ...
            if (role == null)
            {
                return BaseResponse<RegisterCustomerResponse>.Failure("Customer role not found.");
            }
            ...
            return BaseResponse<RegisterCustomerResponse>.Success("Registration Successful.", response);
        }
        public async Task<BaseResponse<RegisterCustomerResponse>> GetCustomerByIdAsync(Guid id)
        {
            ...
            if (customer == null)
            {
                return BaseResponse<RegisterCustomerResponse>.Failure("Customer not found.");
            }

            var response = customer.Adapt<RegisterCustomerResponse>();

            return BaseResponse<RegisterCustomerResponse>.Success("Customer retrieved successfully.", response);
        }
        ...
        public async Task<BaseResponse<ICollection<RegisterCustomerResponse>>> GetAllCustomerAsync()
        {
            ...
            return BaseResponse<ICollection<RegisterCustomerResponse>>.Success("Customers retrieved successfully.", response);
        }
        ...
        public async Task<BaseResponse<UpdateCustomerResponse>> UpdateCustomerAsync(UpdateCustomerRequest request)
        {
            ...
            if (customer == null)
            {
                return BaseResponse<UpdateCustomerResponse>.Failure("Customer not found.");
            }
            ...
            if (request.ProfileImage != null)
            {
                customer.ProfileImageUrl = await _imageService.UploadImageAsync(request.ProfileImage, "customers");
            }
            ...
            return BaseResponse<UpdateCustomerResponse>.Success("Customer updated successfully.", response);
        }
        public async Task<BaseResponse<bool>> DeleteCustomerAsync(Guid id)
        {
            ...
            return BaseResponse<bool>.Success("Customer deleted successfully.", true);
        }
        public async Task<BaseResponse<CustomerDashboardViewModel>> GetDashboardAsync(Guid userId)
        {
            ...
            if (customer == null)
            {
                return BaseResponse<CustomerDashboardViewModel>.Failure("Customer not found.");
            }

            var recipes = await _recipeRepository.GetRecipeByCustomerIdAsync(customer.Id);

            var favourites = await _favouriteRecipeRepository.GetFavouriteRecipeByCustomerIdAsync(customer.Id);

            var comments = await _commentRepository.GetRecipeCommentByCustomerIdAsync(customer.Id);

            var averageRating = await _ratingRepository.GetAverageCustomerRatingByIdAsync(customer.Id);
            ...
            var dashboard = new CustomerDashboardViewModel
            {
                ...
                RecentRecipes = recipes.OrderByDescending(x => x.CreatedBy).Take(5).Adapt<ICollection<CreateRecipeResponseModel>>()
            };

            return BaseResponse<CustomerDashboardViewModel>.Success("Dashboard loaded successfully.", dashboard);
        }
